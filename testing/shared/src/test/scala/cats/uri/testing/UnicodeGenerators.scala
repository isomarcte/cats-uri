...
private[testing] object UnicodeGenerators {
  ...
  /**
   * Utility method for converting a byte into a percent encoded
   * representation.
   *
   * This does not imply that it is valid. In fact we use it to create invalid
   * UTF-8 byte sequences and ensure the PercentDecoder handles them properly.
   */
  ...
  def shuffleCaseGen(value: Gen[String]): Gen[String] =
    value.flatMap{value =>
      stringToSequenceOfCodePoints(value).foldLeft(Gen.const(Vector.empty[Int])){
        case (acc, value) =>
          for {
            acc <- acc
        ...
        new String(codePoints.toArray, 0, codePoints.length)
      )
    }
  ...
  val genOverlongUTF8Encodings: Gen[String] = {
    val genOverlongEncodingsFor1ByteValues: Gen[List[Byte]] =
      Gen.choose(0, 0x7f).flatMap(codePoint =>
        Gen.oneOf(
    ...
    val genOverlongEncodingsFor2ByteValues: Gen[List[Byte]] =
      Gen.choose(0x80, 0x7ff).flatMap(codePoint =>
        Gen.oneOf(
          // The overlong three byte encoding.
          List(0xe0, (codePoint >> 6) | 0x80, ((codePoint & 0x3f) | 0x80)),
          // The overlong four byte encoding.
    ...
    val genOverlongEncodingsFor3ByteValues: Gen[List[Byte]] =
      Gen.choose(0x800, 0xffff).flatMap(codePoint =>
        // The overlong four byte encoding.
          List(0xf0, (codePoint >> 12) | 0x80, ((codePoint >> 6) & 0x3f) | 0x80, (codePoint & 0x3f) | 0x80)
      ).map(_.map(_.toByte))
  ...
  val genInvalidPercentEncodedString: Gen[String] =
    shuffleCaseGen(
      genInvalidUnicodeByteSequence.map(bytes =>
        bytes.map(byteToPercentHexString).mkString
      )
    )
